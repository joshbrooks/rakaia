...
class TestTheParameterRefusals:
    ...
    def test_the_empty_check_comes_before_the_count_check(self):
        """`?offset=&offset=x` is wrong twice; only the first refusal is sent."""
        assert read_param_error(offset="", live=None, offset_count=2) == (
            400,
            b"Empty offset parameter",
        )
