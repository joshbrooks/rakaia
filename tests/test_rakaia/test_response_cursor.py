"""Tests for rakaia.response_cursor."""

from __future__ import annotations

from unittest.mock import patch

from rakaia.response_cursor import (
...
class TestCalculateCursor:
    ...
    def test_default_options(self):
        with patch(
            "rakaia.response_cursor.time.time", return_value=DEFAULT_CURSOR_EPOCH + 40.0
        ):
    ...
    def test_custom_epoch(self):
        ...
        with patch("rakaia.response_cursor.time.time", return_value=1100.0):
            # (1100 - 1000) / 20 = 5
            assert calculate_cursor(opts) == "5"

    def test_at_epoch_returns_zero(self):
        with patch(
            "rakaia.response_cursor.time.time", return_value=DEFAULT_CURSOR_EPOCH
        ):
...
class TestGenerateResponseCursor:
    ...
    def test_client_behind_returns_current(self):
        with patch(
            "rakaia.response_cursor.time.time",
            return_value=DEFAULT_CURSOR_EPOCH + 100.0,
        ):
