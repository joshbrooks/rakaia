...
class TestStreamCreation:
    ...
    def test_create_stream_returns_201(self, client: Client, stream_id: str):
        ...
        response = client.put(f"/protocol/streams/{stream_id}/create", content_type="application/json")

        assert response.status_code == 201

    def test_create_stream_returns_next_offset_header(self, client: Client, stream_id: str):
    ...
    def test_create_existing_stream_returns_200(self, client: Client, stream_id: str):
        ...
        # First create
        client.put(f"/protocol/streams/{stream_id}/create", content_type="application/json")
...
class TestReadOperations:
    ...
    def test_read_with_offset_returns_events_after(self, client: Client, stream_id: str):
        ...
        # Read from offset 2
        response = client.get(f"/protocol/streams/{stream_id}/read?offset=0_0000000000000002")
...
class TestSSEStreaming:
    ...
    @pytest.mark.skip(reason="SSE streaming tests freeze")
    def test_sse_format_follows_spec(self, client: Client, stream_id: str):
        ...
        response = client.get(f"/protocol/streams/{stream_id}/sse?cursor=0_0000000000000000")

        # For streaming responses, concatenate chunks
    ...
    @pytest.mark.skip(reason="SSE streaming tests freeze")
    def test_sse_cursor_resumption(self, client: Client, stream_id: str):
        """SSE with cursor only returns events after cursor."""
        client.put(f"/protocol/streams/{stream_id}/create", content_type="application/json")
        ...
        # Read from cursor after event 2
        response = client.get(f"/protocol/streams/{stream_id}/sse?cursor=0_0000000000000002")

        content = b"".join(response.streaming_content).decode("utf-8")
        # Should only contain events 3, 4, 5
        assert content.count('"index"') == 3
