...
*(This section was filed under `0.3.1` until 2026-09-23. The migration it
describes ships in `0.4.0` — `v0.3.1`'s tree stops at `0009` — so anyone
following it from the `0.3.1` notes applied a migration a release early.)*
...
`closed_by`. It carries no messages at all — read the stream with `read()`. (This
paragraph said the `messages` list "is always empty" until 2026-09-23, which
contradicted the section below removing the field in this same release.)
