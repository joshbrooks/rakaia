...
- The outcome record and the consume loop (`consume`, `Outcome`, the three
  ...
  missing now is simply the example: no demo drives `consume()`, and
  `examples/protocol_streams/demo.py` still hand-rolls poll/apply/commit, which
  is the loop `consume()` was written to replace.
