...
class StreamEventAdmin(admin.ModelAdmin):
    ...
    list_display = ["id", "event_type_badge", "data_preview", "stream_count", "created_at"]
    ...
    def streams_list(self, obj):
        ...
        if not streams:
            return "-"
        return format_html(
            "<br>".join(f'<code>{s}</code>' for s in streams)
        )
...
class StreamEntryAdmin(admin.ModelAdmin):
    ...
    list_display = ["stream_link", "offset", "event_link", "event_type_badge", "created_at"]
    ...
    def stream_link(self, obj):
        ...
        return format_html(
            '<a href="{}">{}</a>', url, obj.stream.stream_id
        )
...
def register_stream_event_admin(event_model_class):
    ...
    if hasattr(event_model_class._meta, "abstract") and event_model_class._meta.abstract:
        return
